...
namespace GrobExp.Compiler.Tests.AssignTests.PreIncrementAssign
{
    ...
    public class TestInstanceMember
    {
        ...
        public void TestIntProp()
        {
            ...
            Expression<Func<TestClassA, int>> exp = Expression.Lambda<Func<TestClassA, int>>(Expression.PreIncrementAssign(Expression.MakeMemberAccess(a, typeof(TestClassA).GetProperty("IntProp"))), a);
        ...
        {
            ParameterExpression a = Expression.Parameter(typeof(TestClassA), "a");
            Expression<Func<TestClassA, double>> exp = Expression.Lambda<Func<TestClassA, double>>(Expression.PreIncrementAssign(Expression.MakeMemberAccess(a, typeof(TestClassA).GetField("DoubleField"))), a);
            var f = LambdaCompiler.Compile(exp, CompilerOptions.CheckNullReferences);
        ...
        {
            ParameterExpression a = Expression.Parameter(typeof(TestClassA), "a");
            Expression<Func<TestClassA, int?>> exp = Expression.Lambda<Func<TestClassA, int?>>(Expression.PreIncrementAssign(Expression.MakeMemberAccess(a, typeof(TestClassA).GetProperty("NullableIntProp"))), a);
            var f = LambdaCompiler.Compile(exp, CompilerOptions.CheckNullReferences);
